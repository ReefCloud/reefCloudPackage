...
#' to scale up predictions across different tiers, extract covariate effect sizes, compute annual contrasts and saves the summarised results at data tiers.
#'
...
#' @examples
#' scale_up_pred_data("type6")
#' @author Julie Vercelloni
#' @export
scale_up_pred_data <- function(whichModel) {
  cat("\n")
  cat(strrep("-", 70), "\n")
  cat("scale_up_pred_data: Starting (model type = ", whichModel, ")\n", sep = "")
  cat(strrep("-", 70), "\n")

  # ---- Validate inputs ----
  if (!whichModel %in% c("type5", "type6")) {
    stop(sprintf("scale_up_pred_data: Unsupported model type '%s'. Only type5/type6 implemented.", whichModel))
  }
  ...
  if (length(missing_globals) > 0) {
    stop(sprintf("scale_up_pred_data: Missing required globals: %s", paste(missing_globals, collapse = ", ")))
  }

  # ---- Find model output files ----
  modelled_dir <- paste0(DATA_PATH, "modelled")
  if (!dir.exists(modelled_dir)) {
    stop(sprintf("scale_up_pred_data: Model directory does not exist: %s", modelled_dir))
  }
  ...
  if (length(files) == 0) {
    stop(sprintf("scale_up_pred_data: No model output files (FRK|INLA) found in %s", modelled_dir))
  }
  ...
  if (!file.exists(tiers_lookup_path)) {
    stop(sprintf("scale_up_pred_data: tiers.lookup.RData not found at %s", tiers_lookup_path))
  }
  ...
  cat(sprintf(
    "  Combined Tier4 aggregation: %s rows (data-only)\n",
  ...
  for (tierIndex in seq(as.numeric(BY_TIER), 2)) {
    ...
    pred_tierIndex <- tryCatch(
      ...
      error = function(e) {
        stop(sprintf("scale_up_pred_data: Failed computing contrasts for %s: %s", tier_col, e$message))
      }
    )

    # Validate output before writing
    if (nrow(pred_tierIndex) == 0) {
      stop(sprintf("scale_up_pred_data: Contrasts for %s produced 0 rows. Model outputs may be invalid.", tier_col))
    }
    ...
    # ---- Write output CSV ----
    output_file <- paste0(AWS_OUTPUT_PATH, "output_data_tier", tierIndex, ".csv")
    tryCatch(
      readr::write_csv(pred_tierIndex, file = output_file, quote = "none"),
      error = function(e) {
        stop(sprintf("scale_up_pred_data: Failed to write %s: %s", output_file, e$message))
      }
    )
    ...
    if (!is.null(info_region)) {
      info_file <- paste0(AWS_OUTPUT_PATH, "info_data_tier", tierIndex, ".csv")
      tryCatch(
  ...
  if (length(output_files_written) == 0) {
    stop("scale_up_pred_data: No output files were written. Something went wrong in the prediction pipeline.")
  }

  cat("\n")
  cat(strrep("-", 70), "\n")
  cat(sprintf("scale_up_pred_data: COMPLETE. Wrote %d files to %s\n", length(output_files_written), AWS_OUTPUT_PATH))
