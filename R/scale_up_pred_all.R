...
#' to scale up predictions across different tiers, extract covariate effect sizes, compute annual contrasts and saves the summarised results at all tiers.
#'
...
#' @examples
#' scale_up_pred_all("type6")
#' @author Julie Vercelloni
#' @export
scale_up_pred_all <- function(whichModel) {
  cat("\n")
  cat(strrep("-", 70), "\n")
  cat("scale_up_pred_all: Starting (model type = ", whichModel, ")\n", sep = "")
  cat(strrep("-", 70), "\n")

  # ---- Validate inputs ----
  if (!whichModel %in% c("type5", "type6")) {
    stop(sprintf("scale_up_pred_all: Unsupported model type '%s'. Only type5/type6 implemented.", whichModel))
  }
  ...
  if (length(missing_globals) > 0) {
    stop(sprintf("scale_up_pred_all: Missing required globals: %s", paste(missing_globals, collapse = ", ")))
  }

  # ---- Find model output files ----
  modelled_dir <- paste0(DATA_PATH, "modelled")
  if (!dir.exists(modelled_dir)) {
    stop(sprintf("scale_up_pred_all: Model directory does not exist: %s", modelled_dir))
  }
  ...
  if (length(files) == 0) {
    stop(sprintf("scale_up_pred_all: No model output files (FRK|INLA) found in %s", modelled_dir))
  }
  ...
  if (!file.exists(tiers_lookup_path)) {
    stop(sprintf("scale_up_pred_all: tiers.lookup.RData not found at %s", tiers_lookup_path))
  }
  ...
  tier4_agg_data <- list() # per-draw sums at Tier4 level (allTier5s)
  ...
  for (i in seq_along(files)) {
    ...
    obj <- tryCatch(
      readRDS(files[i]),
      error = function(e) {
        stop(sprintf("scale_up_pred_all: Failed to read model file %s: %s", basename(files[i]), e$message))
      }
    )
    ...
    # ---- Tier5 contrasts (computed per-file, all Tier5s) ----
    tier5_data <- tryCatch(
      {
        ...
        if (nrow(pdf_data) > 0) {
          ...
          dplyr::bind_rows(make_contrasts(tmp, "Tier5")) # reefCloudPackage::
        } else {
          NULL
        }
      },
      error = function(e) {
        cat(sprintf("    WARNING: Tier5 all contrasts failed for %s: %s\n", basename(files[i]), e$message))
        NULL
      }
    )
    ...
    # ---- Aggregate to Tier4+ level (all Tier5s) ----
    ...
    rm(pdf, agg_data, data_tier5s)
    gc()
    current_mem <- sum(gc()[, 2])

    peak_mem <- max(peak_mem, current_mem)

   # cat(sprintf("    Done. Memory: %.0f MB\n", sum(gc()[, 2])))
    cat(sprintf("    Done. Memory: %.0f MB | Peak loop: %.0f MB\n",
  ...
  cat(sprintf(
    "  Combined Tier4 aggregation: %s rows (all Tier5s)\n",
    format(nrow(combined_agg_data), big.mark = ",")
  ))

  # ---- Process outputs for all tier levels (ALL Tier5 aggregation) ----
  # Primary outputs use all Tier5 aggregation (matching Julie's approach):
  ...
  cat("\n  Computing predictions and contrasts (all Tier5 aggregation)...\n")
  ...
  for (tierIndex in seq(as.numeric(BY_TIER), 2)) {
    ...
    pred_tierIndex <- tryCatch(
      {
        if (tier_col == "Tier5") {
          ...
          if (nrow(result) == 0) {
            ...
            result |>
              dplyr::rename(
                Mean = value, Lower = .lower, Upper = .upper,
                Fold.Change = fold_change, P.up = prob_up,
                P.down = prob_down, Change = arrow, Model.name = model_name,
                Year = year
              ) |>
              dplyr::select(
                Group, !!sym(tier_col), Year, Mean, Lower, Upper, Fold.Change, P.up, P.down, Change, Model.name
              )
          }
        } else {
          # For Tier4/3/2: aggregate from combined_agg_data (all Tier5s)
          # Compute sum_area from tiers.lookup filtered to all Tier5s
         ...
         #   dplyr::filter(as.character(Tier5) %in% all_data_tier5s) |>
          ...
          predictions <- make_contrasts(tmp, tier_col) # reefCloudPackage::
      ...
      error = function(e) {
        stop(sprintf("scale_up_pred_all: Failed computing contrasts for %s: %s", tier_col, e$message))
      }
    )

    # Validate output before writing
    if (nrow(pred_tierIndex) == 0) {
      stop(sprintf("scale_up_pred_all: Contrasts for %s produced 0 rows. Model outputs may be invalid.", tier_col))
    }
      ...
      info_region <- tryCatch(
        {
          ...
          year_range_info <- combined_agg_data |>
            ...
            dplyr::summarise(
              Year.range = paste0(min(fYEAR_numeric, na.rm = TRUE), "–", max(fYEAR_numeric, na.rm = TRUE)),
              .groups = "drop_last"
            )
          ...
          # FRK.prop / INLA.prop: from model tier
        
          n_frk  <- sum(grepl("FRK", basename(files)))
          n_inla <- sum(grepl("INLA", basename(files)))

          total <- n_frk + n_inla

          FRK.prop  <- round(ifelse(n_frk  > 0 && total > 0, 100 * n_frk  / total, 0), 1)
          INLA.prop <- round(ifelse(n_inla > 0 && total > 0, 100 * n_inla / total, 0), 1)
        ...
        error = function(e) {
          cat(sprintf(
            "    WARNING: info_region computation failed for %s: %s (continuing)\n",
            tier_col,
            e$message
          ))
          NULL
    ...
    # ---- Write output CSV ----
    output_file <- paste0(AWS_OUTPUT_PATH, "output_all_tier", tierIndex, ".csv")
    tryCatch(
      readr::write_csv(pred_tierIndex, file = output_file, quote = "none"),
      error = function(e) {
        stop(sprintf("scale_up_pred_all: Failed to write %s: %s", output_file, e$message))
      }
    )
    ...
    if (!is.null(info_region)) {
      ...
      tryCatch(
        ...
        error = function(e) {
          cat(sprintf("    WARNING: scale_up_pred_all: Failed to write %s: %s (non-fatal)\n", info_file, e$message))
        }
      )
  ...
  if (length(output_files_written) == 0) {
    stop("scale_up_pred_all: No output files were written. Something went wrong in the prediction pipeline.")
  }

  cat("\n")
  cat(strrep("-", 70), "\n")
  cat(sprintf("scale_up_pred_all: COMPLETE. Wrote %d files to %s\n", length(output_files_written), AWS_OUTPUT_PATH))
  cat(sprintf("  Final memory: %.0f MB\n", sum(gc()[, 2])))
  cat(sprintf("  Peak loop memory: %.0f MB\n", peak_mem))
  cat(strrep("-", 70), "\n")
